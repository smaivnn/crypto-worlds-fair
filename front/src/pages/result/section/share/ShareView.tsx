...
const ShareView = ({ onShare, label, hint, disabled }: ShareViewProps) => {
    return (
        <div>
            <button
                type="button"
            ...
            >
                ...
                <span className="relative flex items-center gap-2">
                    <Share className="h-4 w-4 text-white/85 transition-colors duration-300 group-hover:text-white" />
                    {label}
                </span>
            </button>
            {hint ? (
                <p className="mt-2 text-center text-xs text-white/55">{hint}</p>
            ) : null}
        </div>
    );
};
